using GCommon;

namespace COW;

public class IIVCursedEnegy : CSVBaseData, IGetId
{
	public ResourceID[] UnlockHudEffect;

	public string[] DescTipsLong;

	public string[] Icon;

	public string[] Name;

	public uint[] UpgradeItemIDs;

	public uint[] UpgradePrice;

	public string[] DescTips;

	public uint ID;

	public uint ItemID;

	public ResourceID NormalEffect;

	public ResourceID UnlockEffect;

	public uint UnlockPrice;

	public override string GetPrimaryKey()
	{
		return null;
	}

	public uint GetId()
	{
		return 0u;
	}

	public override uint GetUIntKey()
	{
		return 0u;
	}

	public override int GetIntKey()
	{
		return 0;
	}

	public override void DeserializeData(MutableStringBinaryReader bR)
	{
	}

	public string _003C_003EiFixBaseProxy_GetPrimaryKey()
	{
		return null;
	}

	public uint _003C_003EiFixBaseProxy_GetUIntKey()
	{
		return 0u;
	}

	public int _003C_003EiFixBaseProxy_GetIntKey()
	{
		return 0;
	}

	public void _003C_003EiFixBaseProxy_DeserializeData(MutableStringBinaryReader P0)
	{
	}
}
